"""
Block-sparse INT8 attention Triton kernel.
Adapted from https://github.com/lihaoyun6/ComfyUI-FlashVSR_Ultra_Fast / jt-zhang/Sparse_SageAttention_API
...
def _attn_fwd(
    ...
    K_ptrs = (K + (off_z * stride_kz + (off_h // num_kv_groups) * stride_kh)
              + offs_n[None, :] * stride_kn + offs_k[:, None])
    K_scale_ptr = K_scale + k_scale_offset
    K_bid_ptr = K_blkid + k_bid_offset + start_m * stride_kbidk
    V_ptrs = (V + (off_z * stride_vz + (off_h // num_kv_groups) * stride_vh)
              + offs_n[:, None] * stride_vn + offs_k[None, :])
    ...
    if STAGE != 1:
        acc, l_i, _ = _attn_fwd_inner(
            acc, l_i, m_i, q, q_scale, kv_len,
            K_ptrs, K_bid_ptr, K_scale_ptr, V_ptrs, stride_kn, stride_vn,
            start_m, BLOCK_M, HEAD_DIM, BLOCK_N, 2, offs_m, offs_n,
        )
...
def forward(q, k, k_block_id, v, q_scale, k_scale,
